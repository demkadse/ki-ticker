...
def render_index(items, editorial):
    ...
    if editorial:
        ...
        <article class="editorial-card">
            ...
            {video_embed}
            <div style="margin-bottom:20px; opacity:0.8;"><a href="{editorial.get('author_url')}" target="_blank">Zum Urheber</a></div>
            <div class="editorial-expand-wrapper" id="eWrap">
                <div class="editorial-text">{editorial.get('description')}</div>
                <div class="read-more-overlay"></div>
            </div>
            <button class="toggle-btn" id="tBtn" onclick="toggleE()">Mehr lesen</button>
    ...
    for idx, s in enumerate(sorted_sources):
        ...
        for it in grouped[s]:
            ...
            <article class="card" data-content="{it['title'].lower()}">
                ...
                <div class="card-body">
                    <div style="font-size:11px; color:var(--muted); margin-bottom:8px;">{dt}</div>
                    <h3><a href="{it['url']}" target="_blank">{it['title']}</a></h3>
                    <button class="category-btn" style="margin-top:auto; width:100%;" onclick="copyToClipboard('{it['url']}')">Link kopieren</button>
                </div>
            </article>"""
        main_html += f"""<section class="source-section"><div class="source-header"><div class="source-title"><img src="https://www.google.com/s2/favicons?domain={grouped[s][0]['domain']}"> {s}</div><div class="carousel-nav"><button class="nav-btn" onclick="scrollC('c-{idx}', -1)"><i class="fa-solid fa-chevron-left"></i></button><button class="nav-btn" onclick="scrollC('c-{idx}', 1)"><i class="fa-solid fa-chevron-right"></i></button></div></div><div class="news-carousel" id="c-{idx}">{cards}</div></section>"""
    ...
    <script>
    function scrollC(id, d) {{ const c = document.getElementById(id); c.scrollBy({{ left: d * 350, behavior: 'smooth' }}); }}
    ...
    function toggleE() {{ const w = document.getElementById('eWrap'); const b = document.getElementById('tBtn'); w.classList.toggle('expanded'); b.innerText = w.classList.contains('expanded') ? 'Zusammenklappen' : 'Mehr lesen'; }}
