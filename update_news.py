...
def generate_sitemap():
    """Erzeugt eine saubere sitemap.xml nur mit internen URLs"""
    pages = [
        {"loc": "", "priority": "1.0"},
...
def render_index(items):
    ...
    for idx, it in enumerate(items[:120]):
        ...
        <article class="card" data-content="{it["title"].lower()}">
          <div class="img-container"><img src="{img_url}" loading="lazy" onerror="this.onerror=null;this.src='{DEFAULT_IMG}';"></div>
          <div class="card-body">
            ...
            <div class="share-bar"><button onclick="copyToClipboard('{it["url"]}')">🔗 Link kopieren</button></div>
          </div>
        </article>"""
        if (idx + 1) % 12 == 0: html_content += ad_block
    return f"""<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{SITE_TITLE}</title><link rel="stylesheet" href="style.css?v={int(time.time())}"><script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-{ADSENSE_PUB}" crossorigin="anonymous"></script></head><body class="dark-mode"><main class="container"><header class="header"><h1>KI‑Ticker</h1><div class="controls"><input type="text" id="searchInput" placeholder="News durchsuchen..."></div></header>{html_content}<footer class="footer"><p>&copy; {now.year} KI‑Ticker | <a href="impressum.html" style="color:var(--acc)">Impressum</a> | <a href="datenschutz.html" style="color:var(--acc)">Datenschutz</a></p></footer></main><script>function filterNews(t){{const val=t.toLowerCase();document.querySelectorAll('.card').forEach(el=>{{el.style.display=el.getAttribute('data-content').includes(val)?'flex':'none';}});}}document.getElementById('searchInput').oninput=(e)=>filterNews(e.target.value);function copyToClipboard(t){{navigator.clipboard.writeText(t).then(()=>alert('Link kopiert!'));}}</script></body></html>"""
...
def main():
    ...
    save_db(all_data)
    generate_sitemap() # Generiert jetzt nur noch 3 eigene Links
